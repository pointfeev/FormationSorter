...
namespace FormationSorter
{
    public static class Selection
    {
        ...
        public static void SelectFormations(IEnumerable<FormationClass> formationClasses = null, string feedback = null)
        {
            ...
            {
                bool isCorrectFormation = formationClasses is null
                                       || FormationClassUtils.IsFormationOneOfFormationClasses(
                                              formation, formationClasses);
            ...
            foreach (Formation formation in Mission.Current?.PlayerTeam?.FormationsIncludingEmpty)
            {
                ...
                bool shouldInvertSelection
                    = Settings.Instance.InverseSelectKey.IsDefinedAndDown() && wasPreviouslySelected;
                if (isCorrectFormation)
                {
            ...
            if (invertedSelections.Any() || selections.Any(f => f.CountOfUnits > 0))
                InformationManager.DisplayMessage(new InformationMessage(
                                                      $"{(invertedSelections.Any() ? "Unselected" : "Selected")} all {feedback}formations",
                                                      Colors.White, "FormationSorter"));
            else
                InformationManager.DisplayMessage(new InformationMessage(
                                                      $"There are no troops to be selected in any {feedback.Replace("and", "or")}formations",
                                                      Colors.White, "FormationSorter"));
        ...
        private static void SetFormationSelections(List<Formation> selections = null)
        {
            ...
            if (!(orderTroopItemVM is null))
            {
                ...
                _ = typeof(MissionOrderTroopControllerVM).GetCachedMethod("SetSelectedFormation")
            ...
            for (int i = 1; i <= selections.Count - 1; i++)
            {
                ...
                if (!(orderTroopItemVM is null))
                    _ = typeof(MissionOrderTroopControllerVM).GetCachedMethod("AddSelectedFormation")
                                                             .Invoke(troopController,
                                                                     new object[] { orderTroopItemVM });
            }
            SortOrderTroopItemVMs();
        }
        ...
        private static OrderTroopItemVM GetOrderTroopItemVM(Formation formation)
        {
            ...
            OrderTroopItemVM orderTroopItemVM
                = troopController.TroopList.SingleOrDefault(t => t.Formation == formation);
            if (orderTroopItemVM is null && Mission.IsCurrentValid() && selectable)
            {
                orderTroopItemVM = new OrderTroopItemVM(formation,
                                                        item => typeof(MissionOrderTroopControllerVM)
                                                               .GetCachedMethod("OnSelectFormation")
                                                               .Invoke(troopController, new object[] { item }),
                                                        _formation => (int)typeof(MissionOrderTroopControllerVM)
                                                                          .GetCachedMethod("GetFormationMorale")
                                                                          .Invoke(troopController,
                                                                               new object[] { _formation }));
        ...
        private static void SortOrderTroopItemVMs()
        {
            ...
            if (!troopController.TroopList.Any())
                return;
            List<OrderTroopItemVM> sorted = troopController.TroopList.OrderBy(item => item.Formation.Index)
                                                           .ToList();
